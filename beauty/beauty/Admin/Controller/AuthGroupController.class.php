...
class AuthGroupController extends BgController {
    ...
    public function index(){
        ...
        foreach($groupList as $k=>$v){
            $adminInfo=M('AuthGroupAccess')->alias('g')->join('beauty_admin a ON g.uid=a.id')->field('a.username')->where("g.group_id={$v['id']}")->select();
            $str='';
    ...
    public function addMember(){
        if(IS_AJAX){
            ...
            $accessInfo=M('AuthGroupAccess')->table('beauty_admin a,beauty_auth_group_access g')->where(array('g.group_id'=>$gid))->where('a.id=g.uid')->select();
