    ...
    <li><label>收件人<b>*</b></label>
        <div class="vocation">
            <select name="role" class="select1">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="$val.id"><?php echo ($val["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
    </li>
    ...
    $(function(){
        ...
        $('#form').submit(function(){
            if(title($("input[name='title']").val())!=1){
                ...
                $.post("<?php echo U('Message/sendMessage');?>", $('#form').serialize(), function (res) {
                    if (res.status == 1) {
                        layer.msg(res.info, {time: 3000, icon: 6}, function () {
                            location = "<?php echo U('sendMessage');?>";
                         })
                    } else {
