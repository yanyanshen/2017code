    ...
    <script type="text/javascript">
        $(document).ready(function(e) {
            ...
            var getCate=function(pid,name){
                $.post('<?php echo U("Pay/Foote/getCateByPid");?>',{pid:pid},function(res){
                    if(res.status){
            ...
            /*向分类表中添加分类*/
            $('.btn').click(function(){
                $.post('<?php echo U("Pay/Foote/addCateByPid");?>',$("#form1").serialize(),function(res){
                   if(res.status){
                        layer.msg('添加成功',function(){
                            location="<?php echo U('Pay/Foote/index');?>";
                        });
                    }else{
                       layer.msg(res.info,{time:700});
                   }
                })
            });
