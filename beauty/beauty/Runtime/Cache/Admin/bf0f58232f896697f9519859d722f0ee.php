...
<form action="<?php echo U('Pay/Foote/search');?>" method="get" id="form1">
    <ul class="seachform">
    ...
    <table class="tablelist">
        ...
        <td style="font-size: 10px;"><a href="<?php echo U('Pay/Foote/index',array('pid'=>$val['id']));?>"><?php echo ($val["fname"]); ?></a></td>
        ...
        <td style="font-size: 10px;" class="par">
            <a href="<?php echo U('Pay/Foote/editor',array('phid'=>$val['id']));?>" class="tablelink">编辑&nbsp;&nbsp;&nbsp;&nbsp;</a>
            <a  phid="<?php echo ($val['id']); ?>" pid="<?php echo ($val['pid']); ?>" class="tablelink updashow" style="cursor:pointer;"><?php echo ($val['show']==1?'下架':'展示'); ?></a>
            <a href="<?php echo U('Pay/Foote/news',array('id'=>$val['id']));?>" class="tablelink" style="margin-left: 15px;cursor: pointer;"><?php echo ($val['pid']==0?'':'发表文章'); ?></a>
        </td>
...
<script type="text/javascript">
    $(function(){
        $('.updashow').click(function(){
            ...
            pid=$(this).attr('pid');
            cur=$(this);
            $.post('<?php echo U("Pay/Foote/updateshow");?>',{ptid:ptid,pid:pid},function(response){
                if(response.status){
                    if(response.info==1){
                        cur.text('下架');
