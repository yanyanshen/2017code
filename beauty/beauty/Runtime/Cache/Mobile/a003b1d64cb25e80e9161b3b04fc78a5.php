...
<!--点赞的异步-->
    ...
    <script type="text/javascript">
        ...
        function goodslayer() {
            ...
            '<?php if(is_array($goodslist)): $i = 0; $__LIST__ = $goodslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?>'+
            '¥ <input type="text" value="<?php echo ($goods["saleprice"]); ?>" class="saleprice" readonly style="color: red;border:none;width:25%;"/>'+
...
function addcartlayer(){
            layer.open({
                ...
                '<span style="color: red;width: 100%;display: inline-block;text-align: center;" class="price">' +
                '<?php if(is_array($goodslist)): $i = 0; $__LIST__ = $goodslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?>'+
                '¥ <input type="text" value="<?php echo ($goods["saleprice"]); ?>" name="price" class="saleprice" readonly style="color: red;border:none;width:25%;"/>'+
                '<?php endforeach; endif; else: echo "" ;endif; ?>'+
                '</span><br/>'+
                '<span style="margin: 40px 0;">库存<?php echo ($goods["num"]); ?>件</span><br/>'+
                '<span>请选择商品类别</span>'+
            ...
            $('.addconfirm').click(function(){
                $.post("<?php echo U('Mobile/MyCart/addcart');?>",$('#cart').serialize(),function (res) {
                    if (res.status == 1) {
                        layer.open({
                            anim:'up'
                            ...
                            '<img src="/Public/Mobile/goodsdetail/images/close.gif" alt="" style="" class="imgclose"/>' +
                            '</span>'+
                            '<li style="text-align: center;margin: 0 auto;"><img src="/Public/Home/images/ok1.jpg" alt="" width="15px"; style="margin-right: 10px;margin-top: 5px;"/>' +
                            '<span style="text-align: center;margin: 0 auto;font-size: 14px;">宝贝已成功加入购物车</span></li>' +
                            '<li style="margin-top: 15px;">' +
                            '<a href="<?php echo U('Mobile/MyCart/mycart');?>" style="width:40%;float:left;cursor:pointer;">去结算</a>' +
                            '<a href="<?php echo U('Mobile/Index/index');?>" style="width:40%;float:right;cursor:pointer;">继续购物</a></li>' +
                            '</ul>' +
                            '</div>'+';'
                    });
                }
...
<script type="text/javascript">
  function tobuylayer() {
      ...
      '<span style="color: red;width: 100%;display: inline-block;text-align: center;"  class="price">' +
      '<?php if(is_array($goodslist)): $i = 0; $__LIST__ = $goodslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?>' +
      '¥ <input type="text" value="<?php echo ($goods["saleprice"]); ?>" name="price" readonly class="saleprice" style="color: red;border:none;width:25%;"/>' +
      '<?php endforeach; endif; else: echo "" ;endif; ?>' +
      '</span><br/>' +
      '<span style="margin: 40px 0;">库存<?php echo ($goods["num"]); ?>件</span><br/>' +
      '<span>请选择商品类别</span>' +
...
//商品价格随着属性的改变而改变的异步
    $('.liguige em').live('click',function(){
        ...
        ml=$(this).text();
        $.post("<?php echo U('Mobile/Goods/saleprice');?>",{gid:gid,xinghao:ml},function(resinfo){
            if(resinfo.status){
