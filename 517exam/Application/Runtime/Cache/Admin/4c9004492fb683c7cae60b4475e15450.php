...
/*添加分类三级联动*/
      /*  var getCate=function(pid,name){
            $.post('<?php echo U("Pay/TestCategory/getCateByPid");?>',{pid:pid},function(res){
                if(res.status){
...
/*向分类表中添加分类*/
        $('.btn').click(function(){
            $.post('<?php echo U("Admin/TestCategory/addCateByPid");?>',$("#form1").serialize(),function(res){
               if(res.info=='分类名不能为空'){
                   layer.alert('分类名不能为空');
               }if(res.info=='分类名已经存在'){
                    layer.alert('分类名已经存在');
            ...
            <form action="" method="post" id="form1">
                <ul class="forminfo">
                    <li><label>分类名称<b>*</b></label>
                        <input name="cname" type="text" class="dfinput" value=""    style="width:240px;"/>
                    </li>
                    <li><label>&nbsp;</label><input type="button" class="btn" value="马上添加"/></li>
                </ul>
            </form>
