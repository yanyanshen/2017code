           ...
           <!-- <form action="<?php echo U('Pay/Test/search');?>" method="get" id="form1">
            ...
            <table class="tablelist">
                ...
                <?php if(is_array($res)): $k = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>
                        ...
                        <td style="width: 150px;height: 40px"><img src="<?php echo ($server); ?>/Uploads/TestShortPic/<?php echo ($data["picurl"]); ?>100_<?php echo ($data["picname"]); ?>" alt="" width="100" height="60"/></td>
                        ...
                        <td style="font-size: 10px;" class="par">
                            <a href="<?php echo U('Admin/Test/editorShortTest',array('tid'=>$data['id'],'type'=>$data['type'],'ifImg'=>$data['ifimg']));?>" class="tablelink">编辑&nbsp;&nbsp;&nbsp;&nbsp;</a>
                            <a tid="<?php echo ($data['id']); ?>" type="<?php echo ($data['type']); ?>" class="tablelink updashow" style="cursor: pointer;"><?php echo ($data['status']==1?'取消':'展示'); ?></a></td>
    ...
    <script type="text/javascript">
        ...
        $(function(){
            $('.updashow').click(function(){
                tid=$(this).attr('tid');
                type=$(this).attr('type');
                $.post("<?php echo U('Admin/Test/updateshow');?>",{tid:tid,type:type},function(response){
                    if(response.status==1){
                        layer.msg(response.info,{icon:6,time:2000},function(){
                            location="<?php echo U('Admin/Test/short_pic_list');?>";
                        })
                    }else{
