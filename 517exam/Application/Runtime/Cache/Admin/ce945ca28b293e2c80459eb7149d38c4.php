        ...
        <tbody>
            <?php if(is_array($list1)): $k = 0; $__LIST__ = $list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$date): $mod = ($k % 2 );++$k;?><tr>
                    ...
                    <td class="par">
                        ...
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <a href="<?php echo U('Pay/TestCategory/editorCate',array('id'=>$date['id']));?>" class="tablelink">编辑</a>
                    </td>
...
<script>
    $(function(){
        $("#exportdata").click(function(){
            $.post("<?php echo U('Pay/Category/export');?>?time1=<?php echo ($time1); ?>&time2=<?php echo ($time2); ?>&keywords=<?php echo ($keywords); ?>",'',function(res){
                if(res.status==1){
                    window.open("<?php echo U('Pay/Category/export');?>?time1=<?php echo ($time1); ?>&time2=<?php echo ($time2); ?>&keywords=<?php echo ($keywords); ?>");
                }else{
    ...
    $(function(){
        $('.click').click(function(){
            ...
            pid=$(this).attr('pid');
            cur=$(this);
            $.post("<?php echo U('Pay/TestCategory/updateshow');?>",{id:id,pid:pid},function(response){
                if(response.status){
                    if(response.info==1){
