            ...
            <table class="tablelist">
                ...
                <?php if(is_array($res)): $k = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>
                        ...
                        <td style="font-size: 10px;" class="par">
                            <a href="<?php echo U('Admin/Test/editorTest',array('tid'=>$data['id'],'type'=>$data['type'],'ifImg'=>$data['ifimg']));?>" class="tablelink">编辑&nbsp;&nbsp;&nbsp;&nbsp;</a>
    ...
    <script type="text/javascript">
        ...
        $(function(){
            $('.updashow').click(function(){
                tid=$(this).attr('tid');
                $.post("<?php echo U('Admin/Test/updateshow');?>",{tid:tid},function(response){
                    if(response.status==1){
                        layer.msg(response.info,{icon:6,time:2000},function(){
                            location="<?php echo U('Admin/Test/one_pic_list');?>";
                        })
                    }else{
