...
class TCPDF {
	 ...
	 * @param string $unit Member measure unit. Possible values are:<ul><li>pt: point</li><li>mm: millimeter (default)</li><li>cm: centimeter</li><li>in: inch</li></ul><br />A point equals 1/72 of inch, that is to say about 0.35 mm (an inch being 2.54 cm). This is a very common unit in typography; font sizes are expressed in that unit.
	...
	protected function _putcatalog() {
		...
		$as = '<< /Event /Print /OCGs ['.$p.' '.$v.'] /TestCategory [/Print] >> << /Event /view /OCGs ['.$p.' '.$v.'] /TestCategory [/view] >>';
	...
	/**
	 * Converts pixels to Member's Units.
	 * @param int $px pixels
	...
	public function addJavascriptObject($script, $onload=false) {
		++$this->n;
		$this->js_objects[$this->n] = array('n' => $this->n, 'map' => $script, 'onload' => $onload);
		return $this->n;
	}
	...
	protected function _putjavascript() {
		...
		if (!empty($this->js_objects)) {
			foreach ($this->js_objects as $key => $val) {
				$out = $this->_getobj($key)."\n".' << /S /JavaScript /JS '.$this->_textstring($val['map'], $key).' >>'."\n".'endobj';
				$this->_out($out);
			}
		}
	}
	...
	/**
	 * Set Member's Rights for PDF Reader
